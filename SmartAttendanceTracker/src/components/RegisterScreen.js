...
const RegisterScreen = ({ navigation }) => {
  ...
  const handleRegister = async () => {
    try {
        const response = await axios.post('http://192.168.1.243:5001/api/auth/register', {
          name,
          email,
          password,
        });
        ...
        const loginResponse = await axios.post('http://192.168.1.243:5001/api/auth/login', {
          email,
          password,
        });
  
        // Store token with Bearer prefix
        await AsyncStorage.setItem('token', `Bearer ${loginResponse.data.token}`);
